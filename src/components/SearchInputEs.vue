<template>
  <div class="relative">
    <search-focus-es @keyup="focusSearch"></search-focus-es>
...
<script>
import SearchFocusEs from './SearchFocusEs'
...
export default {
  ...
  computed: {
    pages () {
      let result = [];
      const allEsPost = this.$static.allEsPost.edges.map(edge => edge.node);
      allEsPost.forEach(page => {
        result.push({
          path: page.path,
          title: page.title,
