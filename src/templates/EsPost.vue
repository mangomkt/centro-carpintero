<template>
  <Esp :corresponding-post="$page.post.corresponding_post">
    <div class="container-inner single mx-auto my-16">
      <h1 class="text-4xl font-bold leading-tight">{{ $page.post.title }}</h1>
      <div class="text-xl text-gray-600 mb-4">{{ $page.post.date }}</div>
      <div class="flex mb-8 text-sm flex-wrap gap-2">
        <g-link
          :to="etiqueta.path"
          v-for="etiqueta in $page.post.etiqueta"
          :key="etiqueta.id"
          class="bg-gray-300 rounded-full px-4 py-2 mr-4 hover:bg-green-300 hover:text-white">
          {{ etiqueta.title }}
        </g-link>
      </div>
      <div class="markdown-body mb-8" v-html="$page.post.content" />
      <div class="mb-8">
        <g-link to="/es/blog" class="font-bold uppercase">Volver al Blog</g-link>
      </div>
    </div>
  </Esp>
</template>

<page-query>
query esPost ($path: String!) {
  post: esPost (path: $path) {
    title
    date (format: "MMMM D, Y")
    content
    summary
    corresponding_post
    etiqueta {
      title
      path
    }
  }
}
</page-query>

<script>
export default {
  metaInfo() {
    return {
      title: this.$page.post.title
    }
  }
}
</script>

<style src="../css/github-markdown.css" />

